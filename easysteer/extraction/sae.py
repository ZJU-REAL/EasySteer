...
class SAEFeatureExplorer:
    ...
    def __init__(self, api_key: str | None = None):
        ...
        if not self.api_key:
            logger.warning(
                "No Neuronpedia API key provided. Set NP_API_KEY or pass api_key "
                "to the constructor."
            )
    ...
    def extract_decoder_vector(
        ...
        try:
            ...
            if feature_index < 0 or feature_index >= W_dec.shape[0]:
                logger.error(
                    "Invalid feature index: %s. Valid range: 0 to %s",
                    feature_index,
                    W_dec.shape[0] - 1,
                )
                return None
            ...
            logger.info(
                "Extracted decoder vector for feature %s, shape: %s",
                feature_index,
